...
function dpill(x::Vector{Float64}, y::Vector{Float64};
    ...
    y = y[sp]
    indlow = (floor(trim*length(x)) + 1) |> Int
    indupp = (length(x) - floor(trim*length(x))) |> Int
    ...
    Nmax = (max(min(floor(n/divisor), blockmax), 1)) |> Int
    Nval = cpblock(x, y, Nmax, 4)

    ## Estimate sig^2, theta_22 and theta_24 using quartic fits on "Nval" blocks.
    (sigsqQ, th22Q, th24Q) = blkest(x, y, Nval, 4)

    ## Estimate theta_22 using a local cubic fit with a "rule-of-thumb" bandwidth: "gamseh"
    ...
    llow = (floor(proptrun*M) + 1) |> Int
    lupp = (M - floor(proptrun*M)) |> Int
    th22kn = sum((mddest[llow:lupp].^2) .* xcounts[llow:lupp])/n

    ## Estimate sigma^2 using a local linear fit with a "direct plug-in" bandwidth: "lamseh"
    ...
    return (sigsqkn*(b-a)/(2.0*sqrt(pi)*th22kn*n))^(1.0/5.0)
end
